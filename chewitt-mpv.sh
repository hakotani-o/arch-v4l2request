...
# 必要なパッケージのインストール
pacman -S --noconfirm --need sudo pacman-contrib wget base-devel git libjpeg-turbo jack2 ladspa libxkbcommon-x11 qt6-base
...
#yes|pacman -U --noconfirm /ffmpeg-*aarch64.pkg.tar* --overwrite "*"
yes|pacman -U /ffmpeg-*aarch64.pkg.tar* --overwrite "*"
...
sed -i "/'libjpeg'/d" PKGBUILD
sed -i "/'ffmpeg'/d" PKGBUILD
